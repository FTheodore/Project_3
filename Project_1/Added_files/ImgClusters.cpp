...
ImgClusters::ImgClusters(vector<int> * img, int rows, int cols, int clstRows, int clstCols) {
...
void ImgClusters::findClusters(vector<int> * img, int imgRows, int imgCols) {
    ...
    int totalBrightness = std::accumulate(img->begin(),img->end(),0);
...
int ImgClusters::findClustBrightness(vector<int> * img, int imgCols, int x, int y) {
