...
from keras.layers import Conv2D, Reshape, Dense, Flatten
...
def addBottleneck(NN, encdr_layers, latent_dim):
    for layer in encdr_layers:
        if layer['type'] == 'conv':
            last_conv_filters_num = layer['filter_num']

    NN = Flatten()(NN) # add compression layer
    NN = Dense(latent_dim, activation=None, name='latent_layer')(NN) # add latent layer
    NN = Dense(last_conv_filters_num*(7**2), activation=None)(NN) # add decompression layer
    NN = Reshape((7,7,last_conv_filters_num))(NN)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if input('Do you want to load a pre-trained model [y|*]: ') == 'y': # load pre-trained model
            ...
            if input('Add bottleneck layers [y|*]: ') == 'y':
                latent_vec_dim = readVal(1, 'Dimension of latent vector: ')
                NN = addBottleneck(NN, encdr_layers, latent_vec_dim)
